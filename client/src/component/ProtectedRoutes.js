...
function ProtectedRoutes({ children }) {
  ...
  return (
    user && (
      <div dir={i18n.language === 'ar' || i18n.language === 'ku' ? 'rtl' : 'ltr'}>
        <div className="navbar bg-base-100 flex justify-between" style={{ flexShrink: 0 , flexWrap : "nowrap" }}>
          <div className="flex items-center">
            ...
            <ul class="flex font-semibold justify-between items-center gap-2">
            <li class={`md:px-4 md:py-2 cursor-pointer hover:text-indigo-400 ${activeItem === '' ? 'text-indigo-500' : 'text-gray-500'}`} onClick={() => handleItemClick('')}>{t('home')}</li>
            <li class={`md:px-4 md:py-2 cursor-pointer hover:text-indigo-400 ${activeItem === 'user-product' ? 'text-indigo-500' : 'text-gray-500'}`} onClick={() => handleItemClick('user-product')}>{t('products')}</li>
            <li class={`md:px-4 md:py-2 cursor-pointer hover:text-indigo-400 ${activeItem === 'about' ? 'text-indigo-500' : 'text-gray-500'}`} onClick={() => handleItemClick('about')}>{t('about')}</li>
              ...
              <ul tabIndex={0} className="mt-3 z-[1] p-2 shadow menu menu-sm dropdown-content bg-base-100 rounded-box w-52">
                ...
                <li>
                  <a className="justify-between"
                    onClick={() => { if (user.role === "admin" || user.role === "moderator") { navigate("/admin") } else { navigate("/profile") } }}
                  >
                    {t('profile')}
                  </a>
                </li>
